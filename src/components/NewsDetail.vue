<template>
  <article class="news-item">
    <div class="news-content">
      <div  class="news-cover" v-bind="getImg(cover.bg)" :class="{showCover: hasbg}" :style="{backgroundImage:'url('+ cover.bg +')'}">
        <p class="news-title">{{cover.title}}</p>
        <span class="news-cover-source">{{cover.source}}</span>
      </div>
      <div v-html="html"></div>
    </div>
  </article>
...
<script>
  export default {
    ...
    methods: {
	  getExpSrc: function (src) {
		  var reg = /<img[^>]*src[=\'\"\s]+([^\"\']*)[\"\']?[^>]*>/gi; 
		  while (reg.exec(src)) 
			{
				this.getImg(RegExp.$1); 
			}
	  },
	  getImg: function (src) {
      ...
      fillCont: function () {
        this.$http.get('api/4/news/' + this.$route.params.id, {}, {
          headers: {
...
.download-banner-v2 .describe,.download-banner-v2 .title {
	margin-left:97px;
	margin-right:84px;
	line-height:1
}
...
.img-mask {
	...
	width:100%;
	height:100%;
	background:-webkit-gradient(linear,left top,left bottom,color-stop(25%,transparent),color-stop(100%,rgba(0,0,0,.6)));
	background:-webkit-linear-gradient(top,transparent 25%,rgba(0,0,0,.6));
...
@media (max-width:480px) {
	.bottom-recommend,.new-download-banner {
		display:block
	}
	.headline .meta {
	...
	.download-banner,.footer-download,html.show-download-banner .download-banner-v2 {
		display:block
	}
	.icon-arrow-right {
		background-repeat:no-repeat;
...
.header-for-mobile-download-link,.header-for-mobile-download-link:active,.header-for-mobile-download-link:hover,.header-for-mobile-download-link:visited {
	display:block;
...
.bottom-recommend .bottom-recommend-download-link,.bottom-recommend .bottom-recommend-download-link:active,.bottom-recommend .bottom-recommend-download-link:hover,.bottom-recommend .bottom-recommend-download-link:visited {
	margin-top:15px;
	display:block;
	border-radius:2px;
	height:35px;
